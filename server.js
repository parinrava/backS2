...
const dbPath = path.join(__dirname, 'patients.db');  // Save the database in the current project
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'OPTIONS') {
        res.writeHead(200, {
            "Access-Control-Allow-Origin": "*",  // Allow your local frontend
        ...
        req.on('end', () => {
            ...
            db.run(query, [patient.name, patient.age, patient.gender], function (err) {
                if (err) {
                    ...
                    res.end(JSON.stringify({ message: 'Error inserting patient data.' }));
                } else {
                    res.writeHead(200, {
                        "Content-Type": "application/json",
                        "Access-Control-Allow-Origin": "*",  // Allow your local frontend
                    });
                    res.end(JSON.stringify({ message: 'Patient data inserted successfully.' }));
                }
            });
        });
            ...
            if (query.trim().toUpperCase().startsWith('INSERT')) {
                db.run(query, function (err) {
                    if (err) {
                        ...
                        res.end(JSON.stringify({ message: 'Error executing query.' }));
                    } else {
                        res.writeHead(200, {
                            "Content-Type": "application/json",
                            "Access-Control-Allow-Origin": "*",  // Allow your local frontend
                ...
                res.end(JSON.stringify({ message: 'Only INSERT queries are allowed via POST.' }));
            }
        });
